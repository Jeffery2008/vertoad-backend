...
final class ArchiveCronJobTest extends TestCase
{
    public function testArchiveParquetCronJobReportsManifestMetrics(): void
    {
        ...
        $job = new ArchiveParquetJob(new ArchiveJob($repository, 's3://vertoad-archive/raw-events'));
    ...
    {
        $job = new DuckDbColdQueryJob(new ColdQueryService(new InMemoryArchiveRepository(), 's3://vertoad-archive/query-results'));
