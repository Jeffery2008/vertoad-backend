...
final readonly class RegisterAction
{
    ...
    public function __invoke(ServerRequestInterface $request, ResponseInterface $response): ResponseInterface
    {
        ...
        try {
            return $this->json($response, $this->auth->register(
                $this->stringField($body, 'email'),
                $this->stringField($body, 'password'),
                $this->stringField($body, 'display_name'),
            ), 201);
        } catch (InvalidArgumentException $exception) {
            return $this->json($response, ['code' => 'invalid_request', 'message' => $exception->getMessage()], 422);
        } catch (RuntimeException $exception) {
            return $this->json($response, ['code' => 'email_already_registered', 'message' => $exception->getMessage()], 409);
        }
    }
