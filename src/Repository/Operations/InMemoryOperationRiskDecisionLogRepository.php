...
final class InMemoryOperationRiskDecisionLogRepository implements OperationRiskDecisionLogRepositoryInterface
{
    /** @var array<string, OperationRiskDecisionLog> */
    private array $entries = [];

    public function append(OperationRiskDecisionLog $entry): OperationRiskDecisionLog
    {
        $this->entries[$entry->decision_id] = $entry;
    ...
    {
        return $this->entries[trim($decisionId)] ?? null;
    }
    ...
    public function search(array $filters = []): array
    {
        $items = array_values(array_filter(
            $this->entries,
            fn (OperationRiskDecisionLog $entry): bool => $this->matches($entry, $filters),
        ));
        usort(
            $items,
            static fn (OperationRiskDecisionLog $left, OperationRiskDecisionLog $right): int =>
                strcmp($right->occurred_at->format(DATE_ATOM), $left->occurred_at->format(DATE_ATOM))
                    ?: strcmp($right->decision_id, $left->decision_id),
        );
     ...
     */
    private function matches(OperationRiskDecisionLog $entry, array $filters): bool
    {
